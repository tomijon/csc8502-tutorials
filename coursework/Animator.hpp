...
public:
	...
	inline void addPitchStep(float pitch, double time) { this->pitch.push({ time, pitch }); }
	inline void addYawStep(float yaw, double time) { this->yaw.push({ time, yaw}); }
	inline void addRollStep(float roll, double time) { this->roll.push({ time, roll }); }
	...
	inline void addPositionStep(Vector3 position, double time) { positions.push({ time, position }); }
